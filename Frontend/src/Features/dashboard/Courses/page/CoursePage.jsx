      ...
      <div className="relative z-10">
        ...
        <main className="pt-32 pb-24 px-6">
          <section className="max-w-7xl mx-auto py-20 text-center hero-3d-content">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}>
              ...
              <h1 className="font-display text-6xl md:text-8xl lg:text-[110px] leading-[0.9] tracking-tight mb-8 text-white">
                Master Your <span className="italic font-serif text-accent block md:inline mt-2">Craft</span>
              </h1>
          ...
          <section className="max-w-7xl mx-auto py-12">
            {loading ? (
               <div className="flex justify-center items-center h-40"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-accent"></div></div>
            ) : (
              <motion.div
                initial="hidden"
                whileInView="visible"
              ...
              >
                {courses.map((course) => (
                  <motion.div key={course._id} variants={{ hidden: { opacity: 0, y: 20, scale: 0.98 }, visible: { opacity: 1, y: 0, scale: 1 } }} transition={{ duration: 0.4 }} className="group will-change-transform">
          ...
          <ScrollReveal rotateX={5} y={20}>
            <section className="mt-32"><div className="glass rounded-[48px] p-1 border-white/5 bg-linear-to-b from-white/5 to-transparent"><ComparisonSection /></div></section>
          </ScrollReveal>

          <ScrollReveal y={20}>
            <section className="mt-32 max-w-4xl mx-auto"><FAQSection /></section>
          </ScrollReveal>

          <ScrollReveal scale={0.98}>
            <section className="mt-32"><CTASection /></section>
          </ScrollReveal>
        </main>
        <Footer />
      </div>
...
function ScrollReveal({ children, rotateX = 0, y = 20, scale = 1 }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y, rotateX, scale }} animate={isInView ? { opacity: 1, y: 0, rotateX: 0, scale: 1 } : {}} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} style={{ transformStyle: "preserve-3d" }} className="will-change-transform">
      {children}
    </motion.div>
  );
}