...
export default function BootcampComparison() {
  ...
  const { media } = useMedia(); // ✅ FIRST define media

  // ✅ NOW create avatars safely
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {comparisonData.map((item, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              key={idx}
              className={`group relative glass p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-white/5 cursor-pointer transition-all duration-500 flex flex-col h-full overflow-hidden ${activeIndex === idx ? 'ring-1 ring-accent/50 bg-accent/[0.02]' : 'hover:border-accent/30'
                }`}
            >
              {/* Gradient Overlay */}
