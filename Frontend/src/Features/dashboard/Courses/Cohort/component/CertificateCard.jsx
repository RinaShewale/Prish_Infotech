...
const CertificateCard = React.forwardRef(({ displayCourseName, displayUserName, displayDate, skillsLearned, isPreview = false }, ref) => {
  return (
    <div
      ref={ref}
      id="certificate-capture-area"
      className={`relative shadow-2xl text-black flex flex-col justify-between ${
        isPreview ? "rounded-[24px] md:rounded-[40px] p-6 sm:p-10 md:p-12" : "p-16"
      }`}
      style={{
        width: isPreview ? "100%" : "1120px",
        height: isPreview ? "100%" : "790px",
        backgroundColor: "#FDFDFD", // MUST be explicit for PDF
        color: "black",
      }}
    >
      {/* Ribbon */}
      ...
      <div className="space-y-4 md:space-y-6">
        ...
        <p className="text-[11px] md:text-sm text-gray-600 leading-relaxed max-w-md">
          ...
          <span className="font-bold text-black"> {skillsLearned}</span> during the 2025 Prish Incubator program.
        </p>
      </div>
      ...
      <div className="mt-8 flex flex-row justify-between items-end gap-4">
        <div>
          <p className="text-xs md:text-lg font-bold mb-0.5 md:mb-1 uppercase text-black">{displayDate}</p>
